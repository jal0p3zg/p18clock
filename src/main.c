...
__CONFIG(__CONFIG2L, _BODEN_CONTROLLED_WITH_SBOREN_BIT_2L
                     & _BODENV_2_7V_2L);
...
static unsigned char _monthdays[]={31,28,31,30,31,30,31,31,30,31,30,31};
...
void uc_init(void)
{
  ...
  ledmtx_init(LEDMTX_INIT_CLEAR|LEDMTX_INIT_TMR0,
	      32, 7, 0xe9, 0xae, 0x88);	/* 32x7@50hz (Fosc=8mhz) */
...
static struct ledmtx_scrollstr_s _sdescriptor={1,1,ledmtx_scrollstr_step,32,0,0,
                                               (__data char *)_tmpstr,
                                               0,1,0x80,ledmtx_scrollstr_stop,0x00};
...
#define IDLE()		do { IDLE_ROUTINE(); Sleep(); } while(0)
...
#define BLINK(arg1,arg2)	((TMR1H&0x40)&&(arg!=CLASS_INPUT)?arg1:arg2)
...
void s_time(char arg, __data char *input) /* __wparam */
{
  if (input==(__data char *)NULL)
    {
      LEDMTX_HOME();
      printf(BLINK("%02hu %02hu","%02hu:%02hu"), _time.hour, _time.min);
    }
...
void s_date_setmday(char arg, __data char *input) /* __wparam */
{
  if (input==(__data char *)NULL)
    {
      LEDMTX_HOME();
      printf(BLINK(MDAYF "  ",MDAYF "%02hu"), _time.mday);
    }
  else if (*input==PUSHB2)
    {
      _state=STATE_DATE;
    ...
    {
      _time.mday=(_time.mday==_monthdays[_time.mon-1])?1:(_time.mday+1);
    }
  else if (*input==PUSHB4)
    {
      _time.mday=(_time.mday==1)?_monthdays[_time.mon-1]:(_time.mday-1);
    }
}
...
void s_date_setmon(char arg, __data char *input) /* __wparam */
{
  if (input==(__data char *)NULL)
    {
      LEDMTX_HOME();
      printf(BLINK(MONF "   ",MONF "%s"), _month[_time.mon-1]);
    }
  else if (*input==PUSHB2)
    {
...
void s_date_setyear(char arg, __data char *input) /* __wparam */
{
  if (input==(__data char *)NULL)
    {
      LEDMTX_HOME();
      printf(BLINK(YEARF "    ",YEARF "%04u"), _time.year);
    }
  else if (*input==PUSHB2)
    {
...
void s_temp_setvdd(char arg, __data char *input) /* __wparam */
{
  if (input==(__data char *)NULL)
    {
      LEDMTX_HOME();
      printf(BLINK(VDDF "   ",VDDF "%03u"), _lm35vdd);
    }
  else if (*input==PUSHB2)
    {
...
void s_temp_setoff(char arg, __data char *input) /* __wparam */
{
  if (input==(__data char *)NULL)
    {
      LEDMTX_HOME();
      printf(BLINK(OFFF "   ",OFFF "%02d"), _lm35off);
    }
  else if (*input==PUSHB2)
    {
      _state=STATE_TEMP;
...
typedef void (*s_routinet)(char,__data char *) /* __wparam */;

static s_routinet __code _stab[]={s_time,s_date,s_temp,s_alarm,s_auto,
	                          s_time_sethour,s_time_setmin,
	                          s_date_setmday,s_date_setmon,s_date_setyear,
	                          s_temp_setvdd,s_temp_setoff,
...
void main(void)
{
  ...
  sprintf(_tmpstr, INITF,
	  *((__code unsigned char *)__IDLOC0),
  ...
  while(1)
      ...
      if (++rcounter==0)
	{
	  ...
	  _stab[_state](MESSAGE_CLASS(c), NULL);	/* NULL -> refresh display */
	}
      ...
      if (c!=-1)
	{
	  switch (c)
		...
		{
		  _stab[_state](MESSAGE_CLASS(c), &c);	/* input to state machine */
